...
const Home = () => {
    ...
    const [enabledHelp, setEnabledHelp] = useState('disabled');
    const onChange = () => {
        if (enabledHelp === '')
            setEnabledHelp('disabled');
        else
            setEnabledHelp('');
    }
    const redirectFormaPagamento = () => {
        <Link to={`/formapagamento/${amount}`} />
    }
    ...
    useEffect(()=>{
        if(produto?.id !== undefined){
            setEnabled(false);
            setProdutos(produtos.concat({id: produto?.id!,
                name: produto?.name!,
        ...
        <>
            <div className="home">
                ...
                <div className='container-center'>
                    ...
                    <Button disabled={false} color='colorButtonBlue' text='F2 - Pesquisar' onSubmit={() => { }} />
                    ...
                    <div className="container-botton">
                        <BottonPage disabled={enabled} onChange={() => onChange()} redirect={() => redirectFormaPagamento()} amount={amount} />
                    </div>
                </div>
            </div>
            <Help enabled={enabledHelp} onChange={() => onChange()} />
        </>
    )
}
